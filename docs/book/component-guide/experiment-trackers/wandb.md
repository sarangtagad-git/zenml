...
By default, ZenML leaves W&B run IDs unset (`run_id_strategy="wandb_generated"`), preserving W&B-generated IDs. If you want deterministic IDs, choose one of the opt-in strategies:

```python
wandb_settings = WandbExperimentTrackerSettings(
    run_id_strategy="step_run",  # One W&B run per ZenML step-run attempt
)

retry_collapsed_settings = WandbExperimentTrackerSettings(
    run_id_strategy="pipeline_step",  # Retries resume one W&B run
)
```

`step_run` creates one deterministic W&B run per ZenML step-run attempt. `pipeline_step` creates one deterministic W&B run for the concrete step or invocation inside a pipeline run, so retries resume the same W&B run and update the latest ZenML step-run ID and version in config. ZenML defaults `resume` to `allow` whenever it passes a deterministic or explicit W&B run ID. `resume="never"` is rejected with `pipeline_step` because retries intentionally reuse the same W&B run ID.

Advanced users can attach logging to an existing W&B run with `run_id="..."`, or pass unmanaged W&B initialization fields through `init_kwargs`. ZenML validates conflicts early: `run_id` cannot be combined with a non-default `run_id_strategy`, `run_config` cannot use reserved `zenml_*` keys, and `init_kwargs` cannot override ZenML-managed init keys such as `entity`, `project`, `name`, `id`, `resume`, `group`, `job_type`, `tags`, `config`, or `settings`.
