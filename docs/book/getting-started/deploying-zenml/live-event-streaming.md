...
{% hint style="warning" %}
Streaming is not persistent storage. Events are size-capped, can be
dropped under load, and disappear when the broker's retention window
elapses. Use run metadata or artifacts for anything you need to keep.
{% endhint %}
...
| `streaming_max_consumers_per_stream` | `maxConsumersPerStream` | `100` | Maximum simultaneous subscribers per run. The 101st subscriber receives `503`. |
...
| `gap` | The subscriber may have missed events between the last `id` and now. Reasons: `outage`, `overflow`, `shutdown`. |
| `error` | A transient server-side error. The client should reconnect with `Last-Event-ID`. |
| `cursor` | A no-payload frame the server emits for filtered-out events and for forward-compatible unknown frame types. Carries an `id:` so `Last-Event-ID` advances. Clients can ignore the `data`. |
...
happened. The next read returns whatever is still retained. If your
client maintains per-event UI state, treat any long disconnect as a
reason to re-read durable state (artifacts, run metadata) rather than
relying on the SSE stream to replay it.
...
| Loss | Events can be lost via producer-side queue overflow (4 096 per process), server-side publish failure (logged, not retried), broker-side `MAXLEN` truncation, or per-subscriber queue overflow. Per-subscriber overflow emits a `gap: overflow` frame. The other loss modes are silent — events drop without a wire-level signal. |
...
- The broker stream is limited per run (default 10 000 entries).
  Subscribers that fall too far behind will see a `gap: truncated` frame.
- The per-run subscriber limit is `streaming_max_consumers_per_stream`
...
**Clients see `gap: truncated` repeatedly.** The subscriber is falling
behind the broker's retention window. Either reduce the producer rate,
raise `ZENML_REDIS_STREAMS_BROKER_MAX_STREAM_LENGTH`, or have the
subscriber discard accumulated state and re-read it on every `gap`.
...
`zenml.streams.publish()` from inside a step or pipeline context. Calls
