...
`zenml.streams.publish()` from inside a step. For enabling streaming on
...
that don't call `zenml.streams.publish()` are unaffected, and calls to
`publish()` against a server with streaming disabled return without
error after the first `501` response.
{% endhint %}
...
elapses. Use [run metadata](../metadata/metadata.md) or
[artifacts](../artifacts/artifacts.md) for anything you need to keep.
{% endhint %}
...
- `payload`: any JSON-serializable dict. Each event is limited to 64 KiB
  on the wire. Larger payloads are rejected by the server.
- `kind`: a label that clients can filter on. Must match
  `[A-Za-z0-9._-]{1,64}`. The names `end`, `gap`, `error`, `cursor`, and
  `system` are reserved for server-emitted control frames and are
  rejected by `publish()`.
...
ZenML automatically flushes the publisher at the end of every step, so
most users don't need to call `flush()` directly. Call it when you need
to be sure a specific event has reached the server before doing
something else — for example, posting a "ready" event right before
sending an external webhook whose target will consume the stream.
...
- The in-process queue holds up to **4 096 events** per Python process.
  ...
  itself never blocks.
- Each event payload is limited to **64 KiB** on the wire envelope. The
  check runs locally when the event is constructed, so oversize payloads
  fail inside `publish()` rather than after an HTTP round-trip.
...
- After the server returns `501 Not Implemented` (streaming disabled),
  the publisher stops sending for about 5 minutes before trying again.
  A pipeline running against a server with streaming off therefore makes
  at most one failed request per 5-minute window.
