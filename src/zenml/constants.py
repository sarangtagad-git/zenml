...
from zenml.enums import AuthScheme, SSEEventName
...
STEP_NAME_PATTERN = r"^[^\n\r\t]{1,255}$"
STREAM_CORRELATION_ID_PATTERN = r"^[^\n\r\t]{1,128}$"
...
# Clamped against `STREAM_EVENT_MAX_BATCH_SIZE` so a misconfigured env
# var can't push a batch past what the server endpoint accepts.
STREAM_PUBLISHER_BATCH_SIZE: int = max(
    1,
    min(
        STREAM_EVENT_MAX_BATCH_SIZE,
        handle_int_env_var(ENV_ZENML_STREAM_PUBLISHER_BATCH_SIZE, default=64),
    ),
)
