...
class _StreamPublisher(metaclass=SingletonMetaClass):
    ...
    def __init__(self) -> None:
        ...
        self._cached_zen_store: Optional["BaseZenStore"] = None
    ...
    def _collect_batch(self) -> List[StreamEvent]:
        ...
        with self._cond:
            ...
            while self._buf and len(batch) < STREAM_PUBLISHER_BATCH_SIZE:
    ...
    def _resolve_zen_store(self) -> Optional["BaseZenStore"]:
        """Return the process's zen_store, caching it on first resolve.

        Returns:
            The cached zen_store, or None if the client is unavailable.
        """
        if self._cached_zen_store is not None:
            return self._cached_zen_store
        from zenml.client import Client

        try:
            self._cached_zen_store = Client().zen_store
        except Exception:
            logger.exception(
                "Could not resolve ZenML zen_store for stream publishing."
            )
            return None
        return self._cached_zen_store
    ...
    def _send_batch(self, events: List[StreamEvent]) -> None:
        ...
        try:
            ...
            zen_store = self._resolve_zen_store()
