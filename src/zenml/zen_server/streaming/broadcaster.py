...
# the attach rather than replaying full history into every queue.
_INITIAL_CURSOR_RETRIES = 5


class StreamCapacityError(RuntimeError):
    """Raised when a stream's consumer cap is reached at attach time."""


class StreamEndedError(RuntimeError):
    """Raised on attach to a session whose reader has already exited.

    Distinct from `StreamCapacityError` so the endpoint can return a
    clean stale-run close response (gap + end) instead of a 503.
    """
...
class _Session:
    ...
    # the idle-grace window so a re-attach can restart from here without
    # re-probing the broker for `latest_id`.
...
class StreamBroadcaster:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            max_consumers_per_stream: Hard cap on concurrent consumers
                per stream.
            idle_grace_seconds: Keep-alive after the last consumer
    ...
    async def attach(
        ...
        session = await self._claim_consumer_slot(
            stream_key, consumer_id, queue
        )
        return self._stream(session, consumer_id, queue, from_id)
    ...
    async def _stream(
        ...
        try:
            ...
            await self._detach(session, consumer_id)

    async def _claim_consumer_slot(
        self,
        stream_key: str,
        consumer_id: str,
        queue: "asyncio.Queue[YieldItem]",
        ...
        Args:
            stream_key: The broker-side key identifying the stream.
            consumer_id: Unique id for the new consumer.
            queue: Per-consumer queue receiving fan-out items.

        Raises:
            StreamCapacityError: If the broadcaster is shutting down or
                the stream has reached its consumer cap.
            StreamEndedError: If the session reader has already exited.

        Returns:
            The session this consumer has been registered with.
        """
        async with self._lock:
            if self._closing:
                raise StreamCapacityError(
                    "Stream broadcaster is shutting down. Cannot attach."
                )
            session = self._sessions.get(stream_key)
            if session is not None:
                if session.ended:
                    raise StreamEndedError(f"Stream {stream_key} has ended.")
                if session.close_task is not None:
                    session.close_task.cancel()
        ...
        # settings) doesn't block other attaches on this replica.
        ...
        async with self._lock:
            ...
            if session is None:
                ...
                if session.close_task is not None:
                    session.close_task.cancel()
                    session.close_task = None
                # Reader was cancelled at idle-detach to release its broker
                # connection; restart it from the persisted cursor.
                if session.reader_task is None or session.reader_task.done():
                    self._start_reader(session)
            if len(session.consumers) >= self._max_consumers:
                raise StreamCapacityError(
    ...
    async def _detach(self, session: _Session, consumer_id: str) -> None:
        """Remove a consumer and release the broker connection if it was the last.
        ...
        Args:
            session: The session the consumer belonged to.
            consumer_id: The consumer to remove.
        """
        async with self._lock:
            session.consumers.pop(consumer_id, None)
            if session.consumers or session.close_task is not None:
                return
            if session.reader_task is not None:
                session.reader_task.cancel()
    ...
    async def _delayed_close(self, session: _Session) -> None:
        """Drop the session and its broker stream after the idle grace window.
        ...
        async with self._lock:
            ...
            session.close_task = None
            session.closed.set()
            # Reader was already cancelled in `_detach`; nothing to await.
            try:
                await self._broker.delete_stream(session.stream_key)
            except Exception:
                logger.debug(
    ...
    async def _reader_loop(self, session: _Session) -> None:
        """Read from the broker and broadcast to attached consumers.
    ...
    def _broadcast_gap(self, session: _Session, reason: GapReason) -> None:
        ...
        Args:
            session: The session whose consumers to notify.
    ...
    def _broadcast(self, session: _Session, item: YieldItem) -> None:
        """Deliver `item` to every attached consumer of `session`.
    ...
    def _put_or_drop(
        ...
        Args:
            queue: The per-consumer queue to put `item` on.
            item: The event or marker to deliver.
        """
        try:
