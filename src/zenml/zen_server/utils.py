...
async def register_event_handlers() -> None:
    ...
    from zenml.dispatcher import EventDispatcher, SourceLoadableEventHandler
    ...
    if server_config().event_handler_sources:
        ...
        for source in server_config().event_handler_sources:
            ...
            try:
                event_handler_cls: type[SourceLoadableEventHandler] = (
                    source_utils.load_and_validate_class(
                        source=source,
                        expected_class=SourceLoadableEventHandler,
                    )
                )
            except Exception as exc:
                logger.exception(
